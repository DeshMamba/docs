...
description: "Build a Chattrvox campaign: choose an agent, pick a contact source, set calling hours, configure retry logic, and start dialing."
---
...
<Steps>
  ...
  <Step title="Select an agent and set a daily cap">
    ...
    Set a **Daily Cap** — the maximum calls per day (1–1,000). The default is 100. Chattrvox pauses the campaign when this limit is reached and resumes the next calling day.
  </Step>
  <Step title="Select a contact source">
    <Tabs>
      <Tab title="Contact list">
        Select a named list from your contacts. Chattrvox calls every active, non-DNC contact in that list.
      </Tab>
      <Tab title="CSV import">
        Select a CSV file you have already uploaded to Contacts. Chattrvox calls every row not marked do-not-call.
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Configure retry logic (simple campaigns only)">
    Set Chattrvox to retry contacts who were not reached:
  ...
  <Step title="Create the campaign">
    Click **Create Campaign**. The campaign saves with idle status. Chattrvox does not dial anyone until you start it.
  </Step>
</Steps>
