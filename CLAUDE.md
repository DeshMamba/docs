...
- Make Chattrvox the active subject: "Chattrvox sends an SMS", "Chattrvox generates a summary."
- Trust the reader. Skip the hand-holding.

**Validated writing pattern (use this as the reference):**

Opening sentence — one plain statement of what the feature does:
> "Conversations shows every call and SMS with a contact in one place."

Follow with one sentence of plain elaboration. Then use headed sections for each sub-topic.

Each section: lead with what Chattrvox does or what the user does. No preamble. No "this section covers."
...
Example of a bad section:
> ## Transcript
> A full record of the conversation is generated by the system after the call ends. It is labeled by speaker and can be accessed in the thread.
