...
This guide takes you from a blank account to a working AI phone call. You will create a workspace, configure an assistant, connect a phone number, and place a test call. The whole process takes about 15 minutes.
...
<Steps>
  ...
  <Step title="Connect a Twilio phone number">
    Your AI assistant needs a phone number to make and receive calls. Chattrvox uses Twilio as the telephony carrier, so you need to connect your Twilio account first.

    1. Go to **Settings → Integrations**.
    2. Find the **Twilio** card and click **Connect Twilio**.
    3. Enter your **Twilio Account SID** and **Auth Token** from the [Twilio Console](https://console.twilio.com), then add a label for this credential set.
    4. Click **Import from Twilio**. Your phone numbers are imported automatically.
  ...
  <Step title="Make your first call">
    ...
    **Test call from the dashboard:**
    Open the assistant you just created, click the phone icon or **Test Call** button, and enter the phone number you want to call. The assistant will initiate an outbound call immediately.

    **Inbound test:**
    Call the Twilio phone number you assigned to the assistant from your mobile phone. The assistant picks up and responds in real time.
    ...
    <Tip>
      Keep your first system prompt simple — something like "You are a friendly assistant for [Company Name]. Introduce yourself and ask how you can help." You can iterate from there once you confirm the call flow is working end-to-end.
    </Tip>
  </Step>
...
Now that your first call is working, here are the natural next steps depending on what you want to build:
